...
const Admin = () => {
  ...
  const loadUsers = async () => {
    ...
    if (data) {
      ...
      setUsers(enrichedData as UserProfile[]);
    }
  };
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    try {
      ...
      if (attempts && attempts.length > 0) {
        ...
        console.log('Attempt IDs to delete answers for:', attemptIds);
        ...
        if (answersError) {
          ...
          toast.error("Failed to delete user answers");
          return;
        }
        ...
        if (attemptsDeleteError) {
          console.error('Error deleting attempts:', attemptsDeleteError);
          toast.error("Failed to delete user attempts");
          return;
        }
      }
      ...
      // Immediately remove user from the list
      setUsers(prevUsers => prevUsers.filter(u => u.id !== userId));
      // Refresh data to ensure consistency
      await loadUsers();
      await loadUserAttempts(); // Refresh attempts list
  ...
  return (
    <div className="min-h-screen animated-bg pb-20">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="border-b border-border/50 bg-muted/20">
                      <tr>
                        <th className="text-left p-3 md:p-4 text-xs md:text-sm font-medium text-muted-foreground">User</th>
                        <th className="text-left p-3 md:p-4 text-xs md:text-sm font-medium text-muted-foreground hidden md:table-cell">Email</th>
                        <th className="text-center p-3 md:p-4 text-xs md:text-sm font-medium text-muted-foreground">Role</th>
                        <th className="text-right p-3 md:p-4 text-xs md:text-sm font-medium text-muted-foreground hidden lg:table-cell">Joined</th>
                        <th className="text-right p-3 md:p-4 text-xs md:text-sm font-medium text-muted-foreground">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border/50">
                      {users.map((userProfile) => (
                        <tr key={userProfile.id} className="hover:bg-primary/5 transition-colors">
                          <td className="p-3 md:p-4">
                            <div>
                              <p className="font-medium text-sm md:text-base">
                            ...
                            <div className="flex justify-center">
                              {userProfile.user_roles && userProfile.user_roles.length > 0 ? (
                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                  userProfile.user_roles[0].role === 'admin'
                          ...
                          <td className="p-3 md:p-4 text-right">
                            <Button
                              size="sm"
                              variant="ghost"
                              onClick={() => setDeleteUserId(userProfile.id)}
                              className="text-destructive hover:text-destructive hover:bg-destructive/10"
                              disabled={userProfile.user_roles?.some(r => r.role === 'admin')}
                            >
                              <Trash2 className="w-4 h-4" />
                            </Button>
                          </td>
      ...
      <AlertDialog open={deleteUserId !== null} onOpenChange={() => setDeleteUserId(null)}>
        <AlertDialogContent className="glass-card border-destructive/30">
          <AlertDialogHeader>
            <AlertDialogTitle className="font-display text-2xl">Delete User?</AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete the user and all their exam attempts and answers. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
